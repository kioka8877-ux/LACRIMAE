 ...
 *
 * Props:
 *   codex: object — le codex.json généré par F02_FORMAT
 *   videoSrc: string — chemin vers clip_001.mp4
 * ────────────────────────────────────────────────────────────── */
...
export const OmniComposition = ({ codex, videoSrc }) => {
  ...
  return (
    <AbsoluteFill style={{ background: '#000', overflow: 'hidden' }}>
      ...
      {codex.audio_enabled !== false && <Audio src={videoSrc} volume={codex.volume ?? 1} />}

      {/* Layer 1: Vidéo source avec zoom + colorimétrie + slow motion + shake */}
      <AbsoluteFill
        style={{
          transform: `scale(${currentZoom.scale * brutalScale}) translate(${
...
const LogoOverlay = ({ logo, width }) => {
  ...
  return (
    <AbsoluteFill style={{ pointerEvents: 'none' }}>
      ...
      >
        <Img src={logo.src} style={{ width: logoWidth, height: 'auto' }} />
      </div>
    </AbsoluteFill>
  );
};
