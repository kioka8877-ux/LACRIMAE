...
export function normalizeMusicTimeline(value = {}, fps = 30, totalFrames = 300) {
  ...
  const matchCutStart = Math.max(0, Number(music.match_cut_start_frame ?? 0));
  return {
    enabled: music.enabled === true || !!music.audio_file || !!music.audio_src,
    audio_file: music.audio_file || '',
    audio_src: music.audio_src || '',
    duration_seconds: Math.max(0, Number(music.duration_seconds || 0)),
    sample_rate: Number(music.sample_rate || 48000),
    bpm: Number(music.bpm || 0),
    sync_mode: AUDIO_SYNC_MODES.includes(music.sync_mode) ? music.sync_mode : 'manual',
    offset_frames: Number(music.offset_frames || 0),
    beats: beats.map((beat, index) => ({
      id: beat.id || `beat_${String(index + 1).padStart(4, '0')}`,
      time_seconds: Number(beat.time_seconds ?? beat.time ?? 0),
      frame: Number(beat.frame ?? toFrame(beat.time_seconds ?? beat.time ?? 0)),
      strength: Number(beat.strength ?? 1),
      kind: beat.kind || 'beat',
    })),
    loop_in: loopIn,
    loop_out: loopOut,
    loop_count: music.loop_count == null ? 'auto' : Number(music.loop_count),
    loop_enabled: music.loop_enabled !== false,
    climax_time: climaxTime,
    climax_frame: climaxTime == null ? null : toFrame(climaxTime),
    match_cut_start_frame: matchCutStart,
    intro_volume: Number(music.intro_volume ?? 1),
    climax_volume: Number(music.climax_volume ?? 1),
    match_cut_volume: Number(music.match_cut_volume ?? 1),
    fade_in_frames: Math.max(0, Number(music.fade_in_frames || 0)),
    fade_out_frames: Math.max(0, Number(music.fade_out_frames || 0)),
...
export function audioSegmentAtFrame(timeline, frame, fps) {
  ...
  const loopIn = Math.round(music.loop_in * fps);
  const loopOut = Math.max(loopIn + 1, Math.round(music.loop_out * fps));
  const loopLength = loopOut - loopIn;
  const introEnd = music.match_cut_start_frame || music.climax_frame || 0;
  const sourceFrame = music.loop_enabled && localFrame < introEnd
    ? loopIn + ((localFrame - loopIn) % loopLength + loopLength) % loopLength
    : localFrame;
  return {
    sourceStart: sourceFrame / fps,
    volume: Math.max(0, localFrame < introEnd ? music.intro_volume : music.match_cut_volume),
    sourceFrame,
  };
}

export function audioMarkersForFrame(timeline, frame, fps) {
  const music = normalizeMusicTimeline(timeline, fps, timeline?.total_frames);
  return music.beats.filter((beat) => Math.abs(beat.frame - frame) <= 1);
}
...
export function musicTimelineManifest(music, fps, totalFrames) {
  const normalized = normalizeMusicTimeline(music, fps, totalFrames);
  return { schema_version: 'dev7.music-timeline.v1', fps, total_frames: totalFrames, ...normalized };
}
...
export function musicWaveformPoints(beats = [], width = 600, height = 54) {
  ...
  const max = Math.max(...beats.map((b) => Number(b.strength || 1)), 1);
  const step = width / Math.max(1, beats.length - 1);
  return beats.map((beat, index) => {
    const x = Math.round(index * step);
    const amp = Math.max(2, (Number(beat.strength || 1) / max) * (height / 2));
    return `${x},${Math.round(height / 2 - amp)} ${x},${Math.round(height / 2 + amp)}`;
  }).join(' ');
}
